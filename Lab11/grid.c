...
int main(int argc, char* argv[])
{
   ...
   if (argc != 3)
   {
      printf("usage %d direct shift\n",argv[0]);
      exit(1);
   }
   direct = atoi(argv[1]); shift = atoi(argv[2]);
   MPI_Init(&argc, &argv);
   ...
   MPI_Cart_create(MPI_COMM_WORLD,2,dim_sizes,wrap_around,1,&grid_comm);
   MPI_Cart_shift(grid_comm,direct,shift,&source,&dest);

   MPI_Sendrecv(&pid,1,MPI_INT,dest,tag,&pid_from,1,MPI_INT,source,tag,grid_comm,&status);
